...
public:
	...
	void getSubsets(double percent_of_subset_for_training_set, double percent_of_subset_for_test_set,
	                Dataset<FeaturesType, LabelsType> &training_set, Dataset<FeaturesType, LabelsType> &test_set){
		long loop_control = 0;
		...
		while(loop_control < (this->vectorOfLabels.size() * percent_of_subset_for_training_set) / 2){
			...
			if(find(chosen_indexes.begin(), chosen_indexes.end(), a_class_index) == chosen_indexes.end() &&
			   find(chosen_indexes.begin(), chosen_indexes.end(), b_class_index) == chosen_indexes.end()){
